...
describe("MCP", () => {
  ...
  it("schema tool returns types and fields", async () => {
  ...
  it("search with type filter returns matching records", async () => {
    const res = await client.send("tools/call", {
      name: "search",
  ...
  it("search with field filter narrows results", async () => {
    const res = await client.send("tools/call", {
      name: "search",
  ...
  it("transact creates a new record", async () => {
    const res = await client.send("tools/call", {
      name: "transact",
      arguments: {
        records: [
  ...
  it("transact updates an existing record", async () => {
    const res = await client.send("tools/call", {
      name: "transact",
      arguments: {
        records: [{ $ref: "task-mcp-created", title: "Updated via MCP" }],
      },
    });
    expect(res).not.toHaveProperty("error");
